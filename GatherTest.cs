...
namespace LlamaLibrary
{
	public class GatherTest : BotBase
	{
		...
		public TimeSpan TimeLeftInDiadem => DirectorManager.ActiveDirector is null
			? TimeSpan.Zero
			: TimeSpan.FromSeconds(Core.Memory.Read<float>(DirectorManager.ActiveDirector.Pointer + TimerOffset)); //640

		//public TimeSpan TimeLeftInDiadem => DirectorManager.ActiveDirector.Pointer ? a.TimeLeftInDungeon : TimeSpan.Zero; //640
...
//-182.1712, 0.8445628, -298.0846
		...
		//Above orange
		private static readonly Vector3 umbralDuststormAbove = new Vector3(395.1406f, 342.3857f, 582.9224f);

		//above green
		private static readonly Vector3 umbralTempestAbove = new Vector3(-594.2943f, 373.2193f, 443.6449f);

		//above purple
		private static readonly Vector3 umbralLevinAbove = new Vector3(626.1301f, 280.0038f, -408.5651f);

		//above red
		private static readonly Vector3 umbralFlareAbove = new Vector3(-427.9185f, 358.5748f, -597.4927f);
		...
		public async Task GatherWeatherNodes()
		{
			...
			//while (TimeLeftInDiadem > new TimeSpan(0, 3, 0))
			while (DutyManager.InInstance)
			{
				switch (WorldManager.CurrentWeatherId)
				{
					case 133:
						...
						break;
					case 135:
						//if (lastWeather == 135) break; 
						await MineWeather(ClassJobType.Miner, umbralLevinAbove, umbralLevin);
						//standBy = afkSpots[time.Next(0, afkSpots.Length)];
						//await StandBy();
						break;
					case 134:
						//if (lastWeather == 134) break; 
						await MineWeather(ClassJobType.Botanist, umbralDuststormAbove, umbralDuststorm);
						//standBy = afkSpots[time.Next(0, afkSpots.Length)];
						//await StandBy();
						break;
					case 136:
						//if (lastWeather == 136) break; 
						await MineWeather(ClassJobType.Botanist, umbralTempestAbove, umbralTempest);
						//standBy = afkSpots[time.Next(0, afkSpots.Length)];
						//await StandBy();
						break;
					default:
						var r = new Random().Next(5000, 30000);
						Log($"天气不对，等 {r / 1000f:F} 秒后退出");
						if (await Coroutine.Wait(r, () => new[] { 133, 134, 135, 136 }.Contains(WorldManager.CurrentWeatherId)))
						{
							Log($"等着等着天气突然变成 {WorldManager.CurrentWeather} 了！开始采集");
							continue;
						}
						await LeaveDiadem();
						//await Coroutine.Sleep(new Random().Next(2000, 6000));
						break;
		...
		public async Task UseCordial()
		{
			if (Core.Me.CurrentGP < 500)
			{
				...
				var cordial = InventoryManager.FilledSlots.FirstOrDefault(i => i.Item.ChnName.Contains("强心剂"));
		...
		public async Task MineWeather(ClassJobType jobType, Vector3 above, Vector3[] safeSpots)
		{
			...
			await FlyTo2(MathEx.GetRandomPointInCircle(above, 15));
			//var safeSpot = safeSpots.ToList().OrderBy(i => i.DistanceSqr(Core.Me.Location)).Select(i => MathEx.GetRandomPointInCircle(i, 3)).First();
		...
		private static async Task LeaveDiadem()
		{
		...
		internal static async Task<bool> FlyTo2(Vector3 loc)
		{
			...
			await Coroutine.Sleep(1000);
			//while (Core.Me.Location.Y < loc.Y) await Coroutine.Yield();
			MovementManager.StopAscending();

			var sm = new SlideMover();
			while (Core.Me.Location.Distance(loc) > 3)
			{
				sm.MoveTowards(loc);
				await Coroutine.Yield();
			}
			...
			//MoveResult moving = MoveResult.GeneratingPath;
			...
			//	moving = Flightor.MoveTo(loc,100f);
